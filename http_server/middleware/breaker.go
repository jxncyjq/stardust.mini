...
func getHttpBreakerWithConfig(path string, config breaker.GoogleBreakerConfig) breaker.Breaker {
	httpBreakersMu.RLock()
	b, ok := httpBreakers[path]
	httpBreakersMu.RUnlock()
	if ok {
	...
	b = breaker.NewGoogleBreakerWithConfig(config)
	httpBreakers[path] = b
	return b
}
...
// CircuitBreakerWithGoogleConfig 熔断器中间件（按路径隔离，支持配置）。
func CircuitBreakerWithGoogleConfig(config breaker.GoogleBreakerConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		b := getHttpBreakerWithConfig(c.FullPath(), config)
		promise, err := b.Allow()
		if err != nil {
			c.AbortWithStatusJSON(http.StatusServiceUnavailable, gin.H{
				"code": http.StatusServiceUnavailable,
